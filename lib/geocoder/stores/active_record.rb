    ...
    module ActiveRecord
      ...
      module ClassMethods
        def distance_from_sql(location, *args)
          latitude, longitude = Geocoder::Calculations.extract_coordinates(location)
          distance_sql(latitude, longitude, *args) if Geocoder::Calculations.coordinates_present?(latitude, longitude)
        end
        ...
        def bearing_sql(latitude, longitude, options = {})
          options[:bearing] = Geocoder.config.distances unless options.include?(:bearing)
          if options[:bearing]
            method_prefix = using_unextended_sqlite? ? 'approx' : 'full'
            Geocoder::Sql.send(
        ...
        def using_sqlite_with_extensions?
          connection.adapter_name.match(/sqlite/i) &&
            defined?(::SqliteExt) &&
            %w[MOD POWER SQRT PI SIN COS ASIN ATAN2].all? do |fn_name|
      ...
      def reverse_geocode
        do_lookup(true) do |o, rs|
          if r = rs.first
            o.__send__ "#{self.class.geocoder_options[:fetched_address]}=", r.address unless r.address.nil?
            r.address
          end
        end
      end
