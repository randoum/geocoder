...
class LookupTest < GeocoderTestCase
  ...
  def test_search_returns_empty_array_when_no_results
    Geocoder::Lookup.all_services_except_test.each do |l|
      next if %i[abstract_api ipgeolocation nationaal_georegister_nl].include?(l)
  ...
  def test_query_url_contains_values_in_params_hash
    Geocoder::Lookup.all_services_except_test.each do |l|
      if %i[freegeoip maxmind_local telize pointpin geoip2 maxmind_geoip2 mapbox ipdata_co ipinfo_io
            ipapi_com ipregistry ipstack postcodes_io uk_ordnance_survey_names].include? l
        next
      end
  ...
  def test_raises_exception_on_invalid_key
    ...
    # Geocoder::Lookup.all_services_except_test.each do |l|
    %i[bing yandex maxmind baidu baidu_ip amap].each do |l|
      lookup = Geocoder::Lookup.get(l)
  ...
  def test_baidu_ip_api_key
    ...
    assert_match 'ak=MY_KEY',
      g.query_url(Geocoder::Query.new('Madison Square Garden, New York, NY  10001, United States'))
  end
