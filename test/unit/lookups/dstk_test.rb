...
class DstkTest < GeocoderTestCase
  ...
  def test_dstk_query_url
    ...
    assert_equal 
'http://www.datasciencetoolkit.org/maps/api/geocode/json?address=Madison+Square+Garden%2C+New+York%2C+NY&language=en&sensor=false', query.url
  end
  ...
  def test_dstk_query_url_with_custom_host
    ...
    query = Geocoder::Query.new('Madison Square Garden, New York, NY')
    assert_equal 
'http://NOT_AN_ACTUAL_HOST/maps/api/geocode/json?address=Madison+Square+Garden%2C+New+York%2C+NY&language=en&sensor=false', query.url
  end
end
