...
class IpapiComTest < GeocoderTestCase
  ...
  def test_url_with_api_key_and_fields
    ...
    g = Geocoder::Lookup::IpapiCom.new
    assert_equal 'http://pro.ip-api.com/json/74.200.247.59?fields=lat%2Clon%2Cxyz&key=MY_KEY',
      g.query_url(Geocoder::Query.new('74.200.247.59'))
  end

  def test_url_with_fields
    Geocoder.configure(ipapi_com: { fields: 'lat,lon' })
    g = Geocoder::Lookup::IpapiCom.new
    assert_equal 'http://ip-api.com/json/74.200.247.59?fields=lat%2Clon',
      g.query_url(Geocoder::Query.new('74.200.247.59'))
  end
  ...
  def test_use_https_with_api_key
    ...
    g = Geocoder::Lookup::IpapiCom.new
    assert_equal 'https://pro.ip-api.com/json/74.200.247.59?key=MY_KEY',
      g.query_url(Geocoder::Query.new('74.200.247.59'))
  end
end
