...
class ErrorHandlingTest < GeocoderTestCase
  ...
  def test_always_raise_response_parse_error
    ...
    %i[freegeoip google ipdata_co].each do |l|
  ...
  def test_always_raise_timeout_error
    ...
    Geocoder::Lookup.all_services_except_test.each do |l|
      next if %i[maxmind_local geoip2].include?(l) # local, does not use cache
  ...
  def test_always_raise_socket_error
    ...
    Geocoder::Lookup.all_services_except_test.each do |l|
      next if %i[maxmind_local geoip2 maxmind_local geoip2 maxmind_local geoip2].include?(l)
  ...
  def test_always_raise_connection_refused_error
    ...
    Geocoder::Lookup.all_services_except_test.each do |l|
      if %i[maxmind_local geoip2 maxmind_local geoip2 maxmind_local geoip2 maxmind_local geoip2
            maxmind_local geoip2].include?(l)
        next
      end
  ...
  def test_always_raise_host_unreachable_error
    ...
    Geocoder::Lookup.all_services_except_test.each do |l|
      if %i[maxmind_local geoip2 maxmind_local geoip2 maxmind_local geoip2 maxmind_local geoip2
            maxmind_local geoip2 maxmind_local geoip2 maxmind_local geoip2].include?(l)
        next
      end
